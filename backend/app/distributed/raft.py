...
class RaftNode:
    ...
    async def start_election(self):
        ...
        if votes >= needed:
            ...
            # NEW: Execute leader synchronization flow
    ...
    async def _leader_synchronization_flow(self):
        """
        New leader synchronization flow:
        1. Request file lists from all cluster nodes
        2. Unify all lists with leader's local list
        3. Download missing songs to the leader
        4. Check each follower's list
        5. Send missing songs to each follower
        """
        logger.info(f"[LEADER_SYNC] Starting leader synchronization flow")
        ...
        try:
            ...
            if not self.is_leader():
                logger.warning(f"[LEADER_SYNC] No longer leader, aborting synchronization")
                return
            ...
            try:
                # Step 1: Request file lists from all nodes in the cluster
                logger.info(f"[LEADER_SYNC] Step 1: Requesting file lists from {len(self.reachable_peers)} peers")
                
                node_file_lists = {}  # node_id -> list of songs
                ...
                async with httpx.AsyncClient(timeout=30.0) as client:
                    for peer_id in list(self.reachable_peers):
                        ...
                        try:
                            ...
                            logger.info(f"[LEADER_SYNC] Requesting file list from {peer_id}")
                            ...
                            if resp.status_code == 200:
                                ...
                                logger.info(f"[LEADER_SYNC] Received {len(metadata)} songs from {peer_id}")
                            else:
                                logger.warning(f"[LEADER_SYNC] Failed to get file list from {peer_id}: status {resp.status_code}")
                        except Exception as e:
                            logger.error(f"[LEADER_SYNC] Error requesting file list from {peer_id}: {e}")
                
                # Step 2: Get leader's local list and unify with all received lists
                logger.info(f"[LEADER_SYNC] Step 2: Unifying file lists")
                
                our_songs = db.query(Music).all()
                our_songs_by_url = {song.url: song for song in our_songs}
                
                logger.info(f"[LEADER_SYNC] Leader has {len(our_songs)} songs locally")
                
                # Create unified list of all songs in the cluster
                unified_songs = {}  # url -> song_data
                ...
                # Add songs from all peers
                for peer_id, songs in node_file_lists.items():
                    for song_meta in songs:
                        url = song_meta.get("filename")
                        if url and url.startswith("http"):
                            # It's a full URL
                            pass
                        else:
                            # It's just a filename, construct URL
                            # This depends on your URL structure
                            continue
                        
                        # Use the song_name and author as identifiers if URL not available
                        song_name = song_meta.get("song_name")
                        author = song_meta.get("author")
                        filename = song_meta.get("filename")
                        
                        if not filename:
                            continue
                        
                        # Construct a pseudo-URL for comparison
                        pseudo_url = f"/static/music/{filename}"
                        
                        if pseudo_url not in unified_songs:
                            unified_songs[pseudo_url] = {
                                "nombre": song_name,
                                "autor": author,
                                "album": None,
                                "genero": None,
                                "url": pseudo_url,
                                "file_size": song_meta.get("has_file", False),
                                "file_id": filename,
                                "source_node": peer_id
                            }
                
                logger.info(f"[LEADER_SYNC] Unified list contains {len(unified_songs)} unique songs")
                
                # Step 3: Download missing songs to the leader
                logger.info(f"[LEADER_SYNC] Step 3: Downloading missing songs to leader")
                ...
                logger.info(f"[LEADER_SYNC] Leader needs to download {len(songs_to_download)} songs")
                ...
                async with httpx.AsyncClient(timeout=60.0) as client:
                    for song_data in songs_to_download:
                        ...
                        if source_node_id == self.node_id:
                            continue  # Skip our own songs
                        
                        # Find the source peer
                        source_peer = self.peers.get(source_node_id)
                        if not source_peer:
                            logger.warning(f"[LEADER_SYNC] Source node {source_node_id} not found for song {song_data.get('nombre')}")
                            continue
                        
                        try:
                            # Download the file
                            file_url = f"http://{source_peer.address}:{source_peer.port}/internal/file/{file_id}"
                            logger.info(f"[LEADER_SYNC] Downloading {file_id} ({song_data.get('nombre')}) from {source_node_id}")
                            ...
                            if file_resp.status_code == 200:
                                ...
                                logger.info(f"[LEADER_SYNC] Downloaded {file_id} successfully")
                                
                                # Add metadata to database via Raft
                                from app.schemas.music import MusicCreate
                                ...
                                if success:
                                    logger.info(f"[LEADER_SYNC] Added metadata for {song_data.get('nombre')}")
                                else:
                                    logger.error(f"[LEADER_SYNC] Failed to add metadata for {song_data.get('nombre')}")
                            else:
                                logger.warning(f"[LEADER_SYNC] Failed to download {file_id}: status {file_resp.status_code}")
                        except Exception as e:
                            logger.error(f"[LEADER_SYNC] Error downloading {file_id}: {e}")
                
                # Step 4 & 5: Check each follower's list and send missing songs
                logger.info(f"[LEADER_SYNC] Step 4-5: Synchronizing followers")
                
                # Refresh our song list after downloads
                db.expire_all()
                our_songs = db.query(Music).all()
                
                logger.info(f"[LEADER_SYNC] Leader now has {len(our_songs)} songs after downloads")
                
                # For each follower, determine what they're missing
                for peer_id in list(self.reachable_peers):
                    peer = self.peers.get(peer_id)
                    if not peer:
                        continue
                    ...
                    logger.info(f"[LEADER_SYNC] Checking follower {peer_id} (has {len(peer_filenames)} files)")
                    
                    # Determine what this peer is missing
                    songs_to_send = []
                    for song in our_songs:
                        if not song.url:
                            continue
                    ...
                    logger.info(f"[LEADER_SYNC] Follower {peer_id} needs {len(songs_to_send)} songs")
                    
                    # Send missing files to this peer
                    for song in songs_to_send:
                        ...
                        if file_path.exists():
                            ...
                            await self._send_file_to_peer(peer, file_path, metadata)
                            logger.info(f"[LEADER_SYNC] Sent {file_id} ({song.nombre}) to {peer_id}")
                        else:
                            logger.warning(f"[LEADER_SYNC] File {file_id} not found locally, cannot send to {peer_id}")
                
                logger.info(f"[LEADER_SYNC] Leader synchronization flow completed successfully")
                
            finally:
                db.close()
                
        except Exception as e:
            logger.error(f"[LEADER_SYNC] Error during leader synchronization: {e}", exc_info=True)
